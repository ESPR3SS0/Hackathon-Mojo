...
def conv1d_pytorch(input_tensor: torch.Tensor, kernel_tensor: torch.Tensor) -> torch.Tensor:
    ...
    ker_params = {
        'B'   : 1,                  # batch
        'in_channels' : 8,                  # in-channels
        'out_channels': 16,                  # out-channels
        'H'   : 10,                  # input height
        'W'   : 10,                  # input width
        'ker_h'  : 5,                  # kernel height
        'ker_w'  : 5,                  # kernel width
        "input_size": input_tensor.shape[0], 
        "conv_size": kernel_tensor.shape[0],
    }


    conv2d = ops.conv2d[ker_params]
    #conv1d = ops.conv1d[{"input_size": input_tensor.shape[0]}]
    torch.compile(conv2d)(output_tensor, input_tensor, kernel_tensor)
